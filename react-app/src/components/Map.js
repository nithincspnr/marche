...
export default function Map({ activeStates }) {
  useEffect(() => {
    if (activeStates.length === 0) {
      // Clears existing selections on Map
      const statePaths = document.getElementsByClassName("state")[0].childNodes;
      statePaths.forEach((element) => (element.style.fill = "#fcdaa9"));
    }
    // Sets selected states
    activeStates.forEach((state) => {
      if (document.getElementsByClassName(state)?.[0]) {
        document.getElementsByClassName(state)[0].style.fill = "#F59200";
      }
    });
  }, [activeStates]);
